...
ターミナルによっては、物理ホイール 1 ノッチあたり複数のマウススクロールイベントが
送られることがあります。`config.toml` で以下のように正規化できます。

```toml
[scroll]
# 1 ノッチとして扱うスクロールイベント数（最小: 1）
events_per_notch = 6
```

- 未設定時: 従来挙動（同一バーストを 1 移動として扱う）
- `1`: 正規化なし（1 イベント -> 1 移動）
- `2`, `3`, ...: N イベントを 1 ノッチとして扱う
